...
def create_allocation_rule(
    ...
    # Validate target asset belongs to user
...
def update_allocation_rule(
    ...
    """Modifie une règle existante (y compris activer/désactiver)."""
...
def delete_allocation_rule(
    ...
    """Supprime une règle d'allocation."""
    rule = db.query(models.AllocationRule).filter(
        models.AllocationRule.id == rule_id,
        models.AllocationRule.user_id == current.id
    ).first()
...
def simulate_allocation(
    ...
    """Simule la répartition d'un revenu selon toutes les règles actives.
    
    Ne modifie aucune donnée — retourne seulement les montants alloués.
    """
    rules = db.query(models.AllocationRule).filter(
        models.AllocationRule.user_id == current.id,
...
def apply_allocation(
    ...
    """Applique les règles d'allocation et met à jour les soldes des actifs cibles.
    
    Filtre les règles :
    - 'all_income' → toujours appliquées
    - 'category' → appliquées si income_category_id correspond
    """
    ...
    for r in rules:
        ...
        # Update the target asset balance
...
def ai_patrimoine_analysis(
    ...
    """Analyse IA du patrimoine : diversification, fonds d'urgence, recommandations."""
    engine = FinancialAIEngine(db=db, user_id=current.id)
