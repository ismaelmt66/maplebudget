...
export default function BudgetAlertCard({ alert }: Props) {
  const cfg = STATUS_CONFIG[alert.status] ?? STATUS_CONFIG.safe;
  const clampedPct = Math.min(alert.percentage, 100);
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-white/50 mt-0.5">
            {money(alert.spent)} <span className="text-white/30">/</span> {money(alert.budget_limit)}
          </p>
        </div>
        <span
      ...
      <div>
        <div className="flex justify-between text-xs text-white/50 mb-1.5">
          ...
          <span className={alert.remaining < 0 ? "text-red-400" : "text-white/50"}>
            {alert.remaining >= 0 ? `${money(alert.remaining)} restant` : `${money(Math.abs(alert.remaining))} dépassé`}
          </span>
        </div>
