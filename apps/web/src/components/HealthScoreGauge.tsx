...
export default function HealthScoreGauge() {
  ...
  const clss = colorClasses(data.color ?? "green");
  ...
  return (
    <div className="rounded-3xl bg-black/40 backdrop-blur-md border border-white/5 p-6 space-y-5">
      <div className="flex items-center justify-between">
        ...
        >
          {data.label ?? data.grade}
        </span>
      </div>

      <div className="flex justify-center">
        <CircularGauge score={data.score} color={data.color ?? "green"} />
      </div>

      <div className="space-y-3">
        <BreakdownBar label="Épargne" value={data.breakdown.savings_rate} color={data.color ?? "green"} />
        <BreakdownBar label="Budgets" value={data.breakdown.budget_adherence ?? data.breakdown.budget_compliance} color={data.color ?? "green"} />
        <BreakdownBar label="Objectifs" value={data.breakdown.goals_progress ?? data.breakdown.goal_progress} color={data.color ?? "green"} />
      </div>
