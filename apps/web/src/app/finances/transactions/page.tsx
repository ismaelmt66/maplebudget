  ...
  return (
    <main className="mb-container space-y-6 pb-12">
      <section className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between animate-fade-in-up">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight mt-3">Gérer</h1>
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const { apiFetch: _fetch } = await import("@/lib/api");
                    const apiBase = process.env.NEXT_PUBLIC_API_BASE ?? `http://${window.location.hostname}:8000`;
                    const res = await fetch(`${apiBase}/reports/monthly/pdf`, {
                      headers: { Authorization: `Bearer ${getToken()}` },
                    });
                    const html = await res.text();
                    const blob = new Blob([html], { type: "text/html" });
                    window.open(URL.createObjectURL(blob), "_blank");
                  } catch { addToast("Erreur lors de la génération du rapport", "error"); }
                }}
                className="px-4 py-2 rounded-xl bg-violet-500/10 border border-violet-500/20 hover:bg-violet-500/20 flex items-center gap-2 transition-colors text-sm text-violet-300"
